		...
		na.action = na.fail)
{
    ...
    z <- .Fortran(stats:::C_stl, x, n,
		  ...
		  seasonal = double(n),
		  trend = double(n),
		  double((n+2*period)*5), PACKAGE = "stats")
    if(periodic) {
	## make seasonal part exactly periodic
	which.cycle <- cycle(x)
	# z$seasonal <- unname(tapply(z$seasonal, which.cycle, mean)[which.cycle])[1:10]
      z$seasonal <- issdac::make_periodic(z$seasonal, period)[which.cycle]
    }
